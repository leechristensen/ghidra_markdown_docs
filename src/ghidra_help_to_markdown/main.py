...
def read_ghidra_version(ghidra_root: Path) -> Optional[str]:
    """Read the Ghidra version+release from Ghidra/application.properties.

    Returns a string like "12.2_DEV" or "11.3_PUBLIC", or None if the file
    isn't readable. The format matches Ghidra's own dist naming convention
    (e.g. `ghidra_12.2_DEV_20260422_linux_x86_64.zip`).
    """
    props_path = ghidra_root / "Ghidra" / "application.properties"
    ...
    if not version:
        return None
    if release:
        return f"{version}_{release}"
    return version


# Matches an output_dir leaf that's already a versioned destination
# (e.g. ./docs/Ghidra_12.0.4_PUBLIC) so we don't double-nest.
_VERSION_SUFFIX_RE = re.compile(r"^Ghidra_\d+\.\d+")


def apply_version_suffix(output_dir: Path, version: str) -> Path:
    """Nest a Ghidra_<version> directory under output_dir, unless already there.

    `./docs` + `12.0.4_PUBLIC` -> `./docs/Ghidra_12.0.4_PUBLIC`
    `./docs/Ghidra_12.0.4_PUBLIC` (already versioned leaf) -> unchanged
    """
    if _VERSION_SUFFIX_RE.match(output_dir.name):
        return output_dir
    return output_dir / f"Ghidra_{version}"
...
class GhidraHelpConverter:
    ...
    def __init__(
        ...
        self.ghidra_install = ghidra_install
        ...
        # When running from an installed Ghidra (no source tree), help content
        # is read from module jars via pyghidra. We cache it here so the rest
        # of the pipeline doesn't need to re-fetch.
        # Key form mirrors the source-tree paths: "help/topics/<Topic>/<file>.htm".
        self._html_content_cache: dict[str, str] = {}
        # True iff the source argument points at an installed Ghidra rather
        # than a source tree. Switches the loader pipeline.
        self._runtime_mode: bool = False
    ...
    def _is_install_layout(self) -> bool:
        """Detect whether self.ghidra_root is an installed Ghidra (jars only)
        or a source tree (with src/main/help/help/topics/...).

        ``ghidraRun`` is the canonical install signal: it's only generated by
        the build, never present in a source checkout. We check it before the
        source-tree shape because an installed Ghidra can bundle a Skeleton
        extension that has source-tree layout under Extensions/.
        """
        run_script = self.ghidra_root / "ghidraRun"
        props = self.ghidra_root / "Ghidra" / "application.properties"
        if run_script.exists() and props.exists():
            return True
        # No ghidraRun -> source tree (must have at least one help module)
        return False

    def _load_from_runtime(self) -> None:
        """Populate toc_tree, html_files, flat_toc, _html_content_cache from a running Ghidra JVM."""
        from .runtime_help import RuntimeHelp, RuntimeTOCEntry

        install = self.ghidra_install or self.ghidra_root
        rt = RuntimeHelp(install)

        # Build TOC tree mirroring the existing TOCEntry shape.
        # Each runtime entry's target is the URL's inner path, prefixed with "help/" so it
        # matches the rest of the pipeline's "help/topics/..." paths.
        rt_top = rt.walk_toc()

        definitions: dict[str, TOCEntry] = {}

        def convert(rt_entry: "RuntimeTOCEntry") -> TOCEntry:
            target: Optional[str] = None
            anchor: Optional[str] = None
            if rt_entry.is_external and rt_entry.target_url:
                # Preserve "external:..." prefix that the rest of the pipeline expects
                target = rt_entry.target_url
            elif rt_entry.inner_path:
                target = "help/" + rt_entry.inner_path
                anchor = rt_entry.anchor
            entry = TOCEntry(
                id=rt_entry.id,
                text=rt_entry.text,
                target=target,
            ...
            for child in rt_entry.children:
                entry.children.append(convert(child))
            return entry

        root_entries = [convert(e) for e in rt_top]
        # The runtime TOC has a single synthetic root with the real entries as children.
        # Unwrap if necessary to match the source-tree shape (root_entries = top-level items).
        if len(root_entries) == 1 and not root_entries[0].target and root_entries[0].children:
            root_entries = root_entries[0].children

        self.toc_tree = TOCTree(entries=root_entries, definitions=definitions)
        self.html_files = get_unique_html_files(self.toc_tree)
        self.flat_toc = flatten_toc(self.toc_tree)

        # Iterate every topic in the combined map and load its HTML content.
        topic_count = 0
        topic_to_url: dict[str, str] = {}
        for topic in rt.iter_topics():
            if not topic.inner_path:
                continue
            html_path = "help/" + topic.inner_path.split("#", 1)[0]
            if html_path in self._html_content_cache:
                continue
            # Skip non-HTML targets that show up in the combined map (rare)
            tail = html_path.rsplit(".", 1)[-1].lower()
            if tail not in ("htm", "html"):
                continue
            try:
                content = rt.read_text(topic.url)
            except Exception as exc:  # noqa: BLE001
                print(f"  Warning: failed to read {topic.url}: {exc}")
                continue
            self._html_content_cache[html_path] = content
            topic_to_url[html_path] = topic.url
            # Ensure every loaded topic has at least an empty TOC entry list so
            # the rest of the pipeline knows about it.
            if html_path not in self.html_files:
                self.html_files[html_path] = []
            topic_count += 1

        self._runtime_topic_urls = topic_to_url
        self._runtime_help = rt
        self._runtime_mode = True
        print(f"  Loaded {topic_count} topics from {install}")
    ...
    def run(self, module_filter: Optional[str] = None, copy_images: bool = True) -> None:
        ...
        runtime_mode = self._is_install_layout()
        if runtime_mode:
            print("  Detected installed Ghidra layout — reading help from jars via pyghidra")

        # Initialize icon resolver for programmatic icon references.
        # Backed by Ghidra's HelpBuildUtils.locateImageReference via pyghidra,
        # which requires an installed Ghidra (not just the source tree).
        # In runtime mode we always have an install (ghidra_root itself).
        print("\n[0.5/10] Initializing icon resolver (pyghidra)...")
        icon_install = self.ghidra_install or (self.ghidra_root if runtime_mode else None)
        if icon_install is None:
            print("  WARNING: --ghidra-install not provided; programmatic icon refs will use text fallbacks only")
            self.icon_resolver = None
        else:
            self.icon_resolver = IconResolver(self.ghidra_root, install_dir=icon_install)
            print(f"  pyghidra session started against {icon_install}")

        if runtime_mode:
            # Skip source-tree file cache; load everything from the JVM.
            print("\n[1/9] Loading TOC + topics from running Ghidra JVM...")
            ...
            # No filesystem discovery — combined map already has every help ID.
            print("\n[2/9] Skipping file discovery (runtime has complete topic list)")
        else:
        ...
        if copy_images:
            print("\n[8/10] Copying images...")
            if runtime_mode:
                image_count = self._copy_images_from_jars()
            else:
    ...
    def _collect_used_anchors(self) -> None:
        ...
        for html_path in all_html_paths:
            # Runtime mode: read from in-memory cache; source-tree mode: read from disk
            if self._runtime_mode and html_path in self._html_content_cache:
                content = self._html_content_cache[html_path]
            else:
    ...
    def _convert_all_html_files(
        ...
        for html_path, entries in all_files_to_convert.items():
            ...
            # Find the source. In runtime mode we have content in-memory;
            # in source-tree mode we read from disk.
            cached_content: Optional[str] = None
            source_path: Optional[Path] = None
            if self._runtime_mode and html_path in self._html_content_cache:
                cached_content = self._html_content_cache[html_path]
            else:
                source_path = self._find_source_file(html_path)
                if not source_path or not source_path.exists():
            ...
            try:
                ...
                if cached_content is not None:
                    # Pin a synthetic source_path so icon_resolver has a hint for relative refs.
                    # The runtime URL itself isn't a usable filesystem path, but the topic
                    # directory structure is what matters for HelpBuildUtils.locateImageReference.
                    converter.source_path = self.ghidra_root / html_path
                    markdown = converter.convert(cached_content)
                else:
    ...
    def _copy_images_from_jars(self) -> int:
        """In runtime mode, extract help images directly from module jars.

        For each unique jar that hosts a topic URL, iterate the jar entries and
        extract anything under ``help/topics/<topic>/images/`` and ``help/shared/``.
        Mirrors the on-disk layout the source-tree pipeline produces.
        """
        from . import ghidra_session

        copied = 0
        if not getattr(self, "_runtime_topic_urls", None):
            return 0

        # Collect unique jar paths. Topic-hosting jars cover help/topics/*/images/,
        # but shared resources (e.g. shared/note.yellow.png) live in Help.jar and
        # don't appear in any topic URL — scan every jar under the install for those.
        jar_paths: set[Path] = set()
        for url in self._runtime_topic_urls.values():
            m = re.match(r"^jar:file:([^!]+)!", url)
            if m:
                jar_paths.add(Path(m.group(1)))
        # Add framework Help jar(s) where help/shared/* typically lives
        for help_jar in self.ghidra_root.glob("Ghidra/Framework/**/lib/Help*.jar"):
            jar_paths.add(help_jar)

        JarFile = ghidra_session.jclass("java.util.jar.JarFile")
        for jar_path in jar_paths:
            try:
                jar = JarFile(str(jar_path))
            except Exception as e:  # noqa: BLE001
                self.log(f"  Warning: cannot open {jar_path}: {e}")
                continue
            try:
                entries = jar.entries()
                while entries.hasMoreElements():
                    ...
                    if entry.isDirectory():
                        continue
                    # help/topics/<topic>/images/<file>  ->  <topic>/images/<file>
                    # help/shared/<file>                ->  shared/<file>
                    if name.startswith("help/topics/") and "/images/" in name:
                        out_rel = name[len("help/topics/") :]
                    elif name.startswith("help/shared/"):
                        out_rel = "shared/" + name[len("help/shared/") :]
                    else:
                        continue
                    # Restrict to image-like extensions
                    suffix = Path(out_rel).suffix.lower()
                    if suffix not in (".png", ".gif", ".jpg", ".jpeg", ".svg", ".bmp", ".ico"):
                        continue
                    dest = self.output_dir / out_rel
                    if dest.exists():
                        continue
    ...
    def _convert_external_docs(self) -> int:
        ...
        # External HTML docs to convert (pattern -> output path). Globs cover
        # both source-tree (Ghidra/.../src/global/docs/...) and install layout
        # (top-level docs/...).
        external_html_docs = {
            "**/PDB/src/global/docs/README_PDB.html": "docs/README_PDB.md",
            "**/GhidraDocs/InstallationGuide.html": "docs/InstallationGuide.md",
            "docs/README_PDB.html": "docs/README_PDB.md",
            "docs/InstallationGuide.html": "docs/InstallationGuide.md",
            # Older Ghidra releases (e.g. 11.0) ship WhatsNew/ChangeHistory as HTML;
            # 12.x switched these to Markdown (handled in external_md_docs).
            ...
            "docs/WhatsNew.html": "docs/WhatsNew.md",
            "docs/ChangeHistory.html": "docs/ChangeHistory.md",
        }
        ...
        external_md_docs = {
            ...
            "docs/WhatsNew.md": "docs/WhatsNew.md",
            "docs/ChangeHistory.md": "docs/ChangeHistory.md",
        }
        ...
        for pattern, output_path in external_html_docs.items():
            ...
            if matches:
                ...
                try:
                    ...
                    # hand-written links to InstallationGuide.html etc.
                    ...
                    markdown = markdown.replace("](WhatsNew.html)", "](WhatsNew.md)")
                    markdown = markdown.replace("](README_PDB.html)", "](README_PDB.md)")
    ...
    def _generate_master_index(self) -> None:
        ...
        # Try the source-tree filesystem glob first; in install/runtime mode
        # the file lives inside a module jar (e.g. Gui.jar!/images/GHIDRA_1.png)
        # and must be extracted via the icon resolver's classpath path.
        ...
        elif self.icon_resolver and not dest.exists():
            resolved = self.icon_resolver.resolve("images/GHIDRA_1.png")
        ...
        # Only include the "What's New" section for docs that actually exist
        # (DEV installs don't ship WhatsNew.md/ChangeHistory.md, so hardcoding
        # them would produce broken links).
        whats_new_entries: list[str] = []
        if (self.output_dir / "docs" / "WhatsNew.md").exists():
            whats_new_entries.append("- [What's New in Ghidra](docs/WhatsNew.md)")
        if (self.output_dir / "docs" / "ChangeHistory.md").exists():
            whats_new_entries.append("- [Change History](docs/ChangeHistory.md)")
        if whats_new_entries:
            lines.append("## What's New")
            lines.append("")
            lines.extend(whats_new_entries)
            lines.append("")
            lines.append("---")
            lines.append("")
...
Examples:
  ...
  ghidra-help-to-markdown /path/to/ghidra ./docs --no-images --verbose
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to an installed/extracted Ghidra (e.g. ~/ghidra_12.1_DEV). Required for "
        "programmatic icon resolution via Ghidra's HelpBuildUtils. Without it, icons "
    ...
    # Append the Ghidra version to the output directory so multiple versions
    # can coexist and the folder name self-documents what it contains.
    output_dir = args.output_dir
    version = read_ghidra_version(args.ghidra_root)
    if version:
        new_output_dir = apply_version_suffix(output_dir, version)
        if new_output_dir != output_dir:
            print(f"Ghidra version detected: {version}")
            print(f"Output directory: {output_dir} -> {new_output_dir}")
            output_dir = new_output_dir
    else:
        print("WARNING: could not read Ghidra version from application.properties; output dir unchanged")
