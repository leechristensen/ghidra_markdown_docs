...
proceed even if the read-live-memory step fails, or only partially succeeds. Include [State](#state) if you need to know which bytes are up to date in the destination
program.
...
- The **Use overlays** checkbox causes the dialog to propose
...
be modified. See [Use overlays](#use-overlays) above.
