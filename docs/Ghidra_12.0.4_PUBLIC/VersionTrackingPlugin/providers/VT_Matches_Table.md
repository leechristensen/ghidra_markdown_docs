...
| # Conflicting | The number of unique associations with either the same source or same destination address.     This is the number of associations that will become [BLOCKED](#match-status) if you accept the match 	          in this row of the table. |
...
[clear](#match-table-actions) the match before executing the remove action.  The clear
...
[Table Column Filters](../../Trees/GhidraTreeFilter.md#multi-term-filter-examples) for creating complex filters for individual table columns.
