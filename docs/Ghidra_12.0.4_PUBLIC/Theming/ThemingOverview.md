...
The primary goal of theming in Ghidra is to allow users to customize the appearance of the UI
...
Before theming, Ghidra used thousands of hard-coded colors, fonts, and icons. They were
...
Look and Feels, especially dark ones.


Ghidra did allow some customization of the current colors using the Ghidra Options API, but
...
As mentioned, all of Ghidra's icons were hard-coded, with many not suitable for use in a dark
...
1. Replace hard-coded values with a level of indirection for colors, fonts, and icons
using property names/IDs
2. Create the concept of a theme that allows for the property values to change
as a group and along with the current Look and Feel in use
3. Create an API that allows for the introduction of system-level properties that can
be mapped to specific Look and Feel values in order to create reusable groups of properties
4. Provide methods for changing the property values, along with the ability to create,
...
The goal of using indirection is to remove all uses of hard-coded colors, fonts, and icons
...
individual values or entire groups of values with one edit.


### Theme


A theme is simply the object that specifies all the values for colors, fonts, and icons
...
The Java Look and Feel is specified by the active theme. These Look and Feel objects
...
Users will be able create, edit, delete, import, export, and of course switch the
