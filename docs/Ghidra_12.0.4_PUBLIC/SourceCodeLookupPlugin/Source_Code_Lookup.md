...
(**Eclipse** or **Window → Preferences... → GhidraDev → Symbol Lookup → Project Name**). The name used here must match the name of the C++ project
created in above.
...
- The port numbers may not match between Ghidra and Eclipse
  - Check that the Ghidra port (Ghidra Front End GUI: **Edit → Tool Options... → Eclipse Integration → Symbol Lookup Port**) matches the Eclipse port
(**Preferences → GhidraDev → Symbol Lookup → Port**)
- The Eclipse plugin may be using the wrong project
  - Select the C/C++ project to be used by the Eclipse plugin (**Preferences → GhidraDev → Symbol Lookup → Project Name**)
