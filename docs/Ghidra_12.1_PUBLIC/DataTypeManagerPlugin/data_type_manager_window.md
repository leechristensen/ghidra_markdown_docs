...
tree structure. To bring up the window, click on the ![Data Types](../icons/dataTypes.png) icon in the tool bar, or select the **Window → Data Types...** menu option in the
tool.


![Data Type Manager](images/DataTypeManager.png)


## The Data Type Tree


The first level nodes under the root ![Book Shelf Open](../icons/BookShelfOpen.png)
are *archive* nodes. There are two special archive nodes: one for standard ["Built-in"](data_type_manager_description.md#data-types) data types and one for data types that are stored in
...
| ![Closed Book Brown](../icons/closedBookBrown.png) | - Built-in archive data types |
| --- | --- |
| ![Closed Book Red](../icons/closedBookRed.png) | - Program data types |
| ![Closed Book Blue](../icons/closedBookBlue.png) | - Project Archive |
| ![Closed Book Green](../icons/closedBookGreen.png) | - File Archive |


Archive Icon modifiers (These icons are overlayed with the icons above)


| ![Check](../icons/check.png) | - Indicates the archive is modifiable by you and possibly by                       others in your shared project |
| --- | --- |
| ![Checkex](../icons/checkex.png) | - Indicates the archive is currently modifiable only by you |
| ![Check Not Latest](../icons/checkNotLatest.gif) | - Indicates the archive is out of date (someone has checked                       changes for this archive into the repository) |
| ![Small Hijack](../icons/small_hijack.gif) | - Indicates the archive is hijacked (modified without being                       checked out) |
| ![Lockoverlay](images/lockoverlay.png)   | - Indicates the archive is a read only                       version and is not modifiable   |
| ![Small Right Arrow](../icons/smallRightArrow.png)   | - Indicates the archive has changes to                       datatypes that have not been committed back to its source archive. A commit                       is needed. |
| ![Small Left Arrow](../icons/smallLeftArrow.png)   | - Indicates the archive has datatypes from a                       source archive that has been modified. An update is needed. |
| ![Double Arrow](../icons/doubleArrow.png) | - Indicates the archive has datatypes that need updating and                       datatypes that need to be committed. |


Other Icons used in the tree:


| ![Closed Folder](../icons/closedFolder.png) | - Category |
| --- | --- |
| ![Package Development](../icons/package_development.png) | - Built-in data type |
| ![Cstruct](../icons/cstruct.png) | - Structure |
| ![C Union](../icons/cUnion.png) | - Union |
| ![Typedef](../icons/typedef.png) | - Typedef |
| ![Function Def](../icons/functionDef.png) | - Function Definition |
| ![Enum](../icons/enum.png) | - Enum |
| ![Finger Pointer](../icons/fingerPointer.png) | - Pointer |
| ![Emblem Favorite](../icons/emblem-favorite.png) | - Favorite |


Data Type Icon modifiers (These icons are overlayed with the icons above)


| ![Small Right Arrow](../icons/smallRightArrow.png) | - Indicates the data type has changes that need to be committed                       to the source archive |
| --- | --- |
| ![Small Left Arrow](../icons/smallLeftArrow.png) | - Indicates the data type has been updated in the source archive                       and needs to be updated locally. |
| ![Double Arrow](../icons/doubleArrow.png) | - Indicates the data type has changed both in this archive and                       in the source archive. (conflict)   |
| ![Red Question Mark](../icons/redQuestionMark.png) | - Indicates the data type is missing in the source archive.  It is important to                       note that this determination ibased upon a matching Universal ID and not just                        its pathname. |
...
![Conflict Rename](../icons/conflictRename.png) - *Rename New or Moved Data Type* is the default behavior where a unique "*.conflict*" suffix is
appended to the new or moved data type.


![Conflict Keep](../icons/conflictKeep.png) - *Use Existing Data Type* causes the existing data type within the destination category to be used in
...
![Conflict Replace](../icons/conflictReplace.png) - *Replace Existing Data Type* causes the existing data type within the destination category to
...
![Conflict Replace Or Rename](../icons/conflictReplaceOrRename.png) -
...
<a name="navigation_actions"></a> ![Left Alternate](../icons/left.alternate.png)  /
![Right Alternate](../icons/right.alternate.png) -
Navigates forward and backward in the stack of visited data types.


![Collapse All](../icons/collapse_all.png) - Collapses all open
tree nodes, except for the root node.


<a name="show_filter"></a>![Configure Filter](../icons/exec.png) -
...
| **Standard Archive**   | <a name="standard_archives"></a> This menu option will have a               sub-menu item for all file archives contained within the Ghidra installation. Only               those archives contained within a module *data/typeinfo* subdirectory will be included. If               contained within a contrib module, the contrib name will also be displayed. |
| **Recently Opened Archive** | <a name="recent_archives"></a> This menu option will               have a sub-menu item for all recently opened file archives. Choosing one of the               recently open archives actions will open that archive. |
| **Refresh BuiltIn               Types**   | <a name="refresh_builtintypes"></a> This action will search for new [Built-in](data_type_manager_description.md#data-types) data types in the classpath. This is only useful if               somehow new data type java classes have been added since Ghidra was started. |
| **Preview Window**   | <a name="preview_window"></a> This action will show a preview window in the lower                 part of the data types window. This preview window will be updated with a preview                 of the selected data type node. To close the preview window, simply click the **Preview Window** action again. Below is a picture of the data type manager                 with the preview window open. |
...
| **Edit Archive               Paths...**   | <a name="edit_archive_paths"></a> This action will launch a dialog that               can be used to add, remove, or reorder the directories used for searching for a file               archive. Ghidra programs only store the name of the archive. Each directory in the [archive path](data_type_manager_description.md#file-archive-path) is searched in order, looking for a file               archive with that name. See the section on [editing archive paths.](data_type_manager_archives.md#edit-data-type-archive-paths)   |
...
The *\**Merge...*\** action is used to
[merge](data_type_manager_description.md#merging-data-types) a
data type with another data type.
...
Allows the user to associate the selected data type with a new source archive.  This action
...
local menu![Menu16](../icons/menu16.gif) . The following dialog is
displayed.


![Find Data Types](images/FindDataTypes.png)
...
![Find Data Types By Size](images/FindDataTypesBySize.png)
...
This is similar to the **Find Data Types by Size** action except
that it lets you specify one or more enum values (or value ranges) that will be used to
show only enums that contain any of the specified values.
