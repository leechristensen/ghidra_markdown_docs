...
This key binding action shows a dialog to quickly find and run scripts.
The dialog organizes scripts into two sections:


- **Recent Scripts** - Shows up to 10 of the most recently executed scripts in
...
Type any part of a script name or description in the filter field to narrow down the
list. The filter wildcard matching using \* (any characters) and ? (single character). Use
the down arrow keys or mouse to select a script, then press Enter or click OK to run it.


The detail pane on the right displays information about the currently selected script,
including its description, author, category, and assigned keybinding (if any).
