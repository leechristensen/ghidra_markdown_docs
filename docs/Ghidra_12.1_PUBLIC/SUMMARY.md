- [Introduction](Intro/Intro.md)
- Getting Started
    - [Starting Ghidra](Intro/Intro/#starting-ghidra)
    - [Error Dialogs](Intro/Intro/#error-dialogs)
    - [Overview](Intro/Intro/#ghidra-overview)
    - [Docking Windows](DockingWindows/Docking_Windows.md)
    - [Ghidra Project Window](FrontEndPlugin/Ghidra_Front_end/#ghidra-project-window)
    - [Ghidra Server](GhidraServer/GhidraServer/#ghidraserver)
    - [About Ghidra](About/About_Ghidra.md)
- Ghidra Projects
    - [New Project](FrontEndPlugin/Creating_a_Project.md)
    - [Open Project](FrontEndPlugin/Opening_a_Ghidra_Project.md)
    - [Close Project](FrontEndPlugin/Close_Project.md)
    - [Reopen Project](FrontEndPlugin/Re-opening_a_Project.md)
    - [Save Project](FrontEndPlugin/Saving_a_Ghidra_Project.md)
    - [Archive Current Project](FrontEndPlugin/Archive_Project.md)
    - [Restore Project](FrontEndPlugin/Restore_Project.md)
    - [Project Repository](VersionControl/project_repository.md)
    - [Project Access List](FrontEndPlugin/Ghidra_Front_end/#edit-project-access-list)
- Programs
    - [Import Program](ImporterPlugin/importer.md)
    - [Batch Import](ImporterPlugin/importer/#batch-import)
    - [Export Program](ExporterPlugin/exporter.md)
    - [Open Program](ProgramManagerPlugin/Opening_Program_Files.md)
    - [Close Program](ProgramManagerPlugin/Closing_Program_Files.md)
    - [Navigate Programs](ProgramManagerPlugin/Navigating_Program_Files.md)
    - [Rename Program](FrontEndPlugin/Ghidra_Front_end/#rename)
    - [Save Program](ProgramManagerPlugin/Saving_Program_Files.md)
    - [Delete Program](FrontEndPlugin/Ghidra_Front_end/#delete)
    - [About Program](About/About_Program_File.md)
    - [Merge Programs](Repository/Merge_Program_Files.md)
- Tools
    - [Configure Tool](Tool/Configure_Tool.md)
    - [Default Tools](Tool/Ghidra_Tool_Administration/#default-tool)
    - [Tool Options Dialog](Tool/ToolOptions_Dialog.md)
    - [Create Tool](Tool/Ghidra_Tool_Administration/#create-tool)
    - [Run Tool](Tool/Ghidra_Tool_Administration/#run-tool)
    - [Close Tool](Tool/Ghidra_Tool_Administration/#close-tool)
    - [Save Tool](Tool/Ghidra_Tool_Administration/#save-tool-to-tool-chest)
    - [Tool Connections](FrontEndPlugin/Connecting_Tools.md)
    - [Edit Plugin Path](FrontEndPlugin/Edit_Plugin_Path.md)
    - [Import Tools](Tool/Ghidra_Tool_Administration/#import-tool-to-tool-chest)
    - [Import Default Tools](Tool/Ghidra_Tool_Administration/#import-default-tools-to-tool-chest)
    - [Export Tool](Tool/Ghidra_Tool_Administration/#export-tool)
- Ghidra Functionality
    - [Assembler](AssemblerPlugin/Assembler.md)
    - BSim
        - BSim Database Configuration
            - [Overview](BSim/DatabaseConfiguration/#overview)
            - [Server Configuration](BSim/DatabaseConfiguration/#server-configuration)
            - [Creating a Database](BSim/DatabaseConfiguration/#creating-a-database)
            - [Tailoring BSim Meta-data](BSim/DatabaseConfiguration/#tailoring-bsim-metadata)
        - Ingesting Executables
            - [Ingest Process](BSim/IngestProcess/#ingest-process)
            - [Tailoring Analysis](BSim/IngestProcess/#tailoring-analysis)
            - [Analysis Effects on Feature Extraction](BSim/IngestProcess/#analysis-effects-on-feature-extraction)
            - [Maintenance](BSim/IngestProcess/#maintenance)
            - [Migration](BSim/IngestProcess/#migration)
        - BSim Search
            - [Enabling the BSim Search Plugin](BSimSearchPlugin/BSimSearch/#enabling-the-bsim-search-plugin)
            - [Defining And Managing BSim Database Definitions](BSimSearchPlugin/BSimSearch/#defining-and-managing-bsim-databases)
            - [Overview Query](BSimSearchPlugin/BSimSearch/#bsim-overview-query)
            - [Overview Query Results](BSimSearchPlugin/BSimSearch/#bsim-overview-results)
            - [Similar Function Search](BSimSearchPlugin/BSimSearch/#bsim-similar-function-search)
            - [Similar Function Search Results](BSimSearchPlugin/BSimSearch/#similar-function-search-results)
            - [Authentication](BSimSearchPlugin/BSimSearch/#authentication)
        - Features and Weights
            - [Features of Software Functions](BSim/FeatureWeight/#features-of-software-functions)
            - [Weighting Software Features](BSim/FeatureWeight/#weighting-software-features)
            - [Comparing Feature Vectors](BSim/FeatureWeight/#comparing-feature-vectors)
            - [BSim Feature Visualizer](BSimFeatureVisualizerPlugin/BSimFeatureVisualizerPlugin.md)
        - Command-Line Utility Reference
            - [BSim Control (bsim_ctl)](BSim/CommandLineReference/#bsim_ctl)
            - [BSim Command (bsim)](BSim/CommandLineReference/#bsim)
    - Byte Viewer
        - [Formats](ByteViewerPlugin/The_Byte_Viewer/#data-formats)
        - [Byte Editing](ByteViewerPlugin/The_Byte_Viewer/#editing-memory)
        - [Configuration Options](ByteViewerPlugin/ByteViewerOptions.md)
    - Data Type Manager
        - [Basic Concepts](DataTypeManagerPlugin/data_type_manager_description/#basic-concepts)
        - [Data Type Window](DataTypeManagerPlugin/data_type_manager_window.md)
        - [Archives](DataTypeManagerPlugin/data_type_manager_description/#working-with-data-type-archives)
        - [Categories](DataTypeManagerPlugin/data_type_manager_description/#working-with-categories)
        - [Data Types](DataTypeManagerPlugin/data_type_manager_description/#working-with-data-types)
        - [Managing Archives](DataTypeManagerPlugin/data_type_manager_archives.md)
        - [Structure Editor](DataTypeEditors/StructureEditor.md)
        - [Enum Editor](DataTypeEditors/EnumEditor.md)
    - Code Browser
        - [Browser Field Formatter](CodeBrowserPlugin/Browser_Field_Formatter.md)
        - [Markers](CodeBrowserPlugin/CodeBrowser/#markers)
        - [Cursor Text Highlight](CodeBrowserPlugin/CodeBrowser/#cursor-text-highlight)
        - [Configuration Options](CodeBrowserPlugin/CodeBrowserOptions.md)
        - [Program Differences](Diff/Diff.md)
    - Debugger
        - Getting Started
            - [Launching a Target](Debugger/GettingStarted/#launching-a-target)
        - Launchers
            - drgn Integration
                - [Attach](drgn/drgn/#attach)
                - [Core Dump](drgn/drgn/#attach)
                - [Linux Kernel](drgn/drgn/#attach)
            - GDB Integration
                - [gdbserver via SSH](gdb/gdb/#gdbserver-via-ssh)
                - [Local](gdb/gdb/#local)
                - [QEMU](gdb/gdb/#qemu)
                - [Remote](gdb/gdb/#remote)
                - [rr](gdb/gdb/#rr)
                - [Via SSH](gdb/gdb/#via-ssh)
                - [Via SSH (to Windows cmd shell)](gdb/gdb/#via-ssh-to-windows-cmd-shell)
                - [Wine](gdb/gdb/#wine)
            - Java Debugger Integration
                - [Attach b PID](jpda/jpda/#attach-by-pid)
                - [Attach by JDWP](jpda/jpda/#attach-by-jdwp)
                - [Local](jpda/jpda/#local)
            - LLDB Integration
                - [Android (native)](lldb/lldb/#android)
                - [Local](lldb/lldb/#local)
                - [macOS Kernel](lldb/lldb/#macos-kernel)
                - [Remote](lldb/lldb/#remote)
                - [Via SSH](lldb/lldb/#via-ssh)
                - [Via SSH (to Windows cmd shell)](lldb/lldb/#via-ssh-to-windows-cmd-shell)
            - WinDbg (dbgeng.dll) Integration
                - [Attach](dbgeng/dbgeng/#attach)
                - [Extended Local](dbgeng/dbgeng/#extended-local)
                - [Local](dbgeng/dbgeng/#local)
                - [Process Server](dbgeng/dbgeng/#process-server)
                - [Remote](dbgeng/dbgeng/#remote)
                - [Remote via SSH](dbgeng/dbgeng/#remote-via-ssh)
                - [TTD (Time-Travel Debugging)](dbgeng/dbgeng/#ttd-time-travel-debugging)
                - [Windows Kernel](dbgeng/dbgeng/#windows-kernel)
            - x64dbg integration via x64dbg_automate
                - [Attach](x64dbg/x64dbg/#attach)
                - [Local](x64dbg/x64dbg/#local)
                - [Remote via SSH](x64dbg/x64dbg/#remote-via-ssh)
        - [Connection Manager](TraceRmiConnectionManagerPlugin/TraceRmiConnectionManagerPlugin.md)
        - [Troubleshooting](Debugger/Troubleshooting.md)
        - [Debug Console](DebuggerConsolePlugin/DebuggerConsolePlugin.md)
        - [Copy Actions](DebuggerCopyActionsPlugin/DebuggerCopyActionsPlugin.md)
        - [Model](DebuggerModelPlugin/DebuggerModelPlugin.md)
        - [Threads](DebuggerThreadsPlugin/DebuggerThreadsPlugin.md)
        - [Trace Management](DebuggerTraceManagerServicePlugin/DebuggerTraceManagerServicePlugin.md)
        - [Emulation](DebuggerEmulationServicePlugin/DebuggerEmulationServicePlugin.md)
        - [Memory](DebuggerMemoryBytesPlugin/DebuggerMemoryBytesPlugin.md)
        - [Registers](DebuggerRegistersPlugin/DebuggerRegistersPlugin.md)
        - [Dynamic Listing](DebuggerListingPlugin/DebuggerListingPlugin.md)
        - [Disassembly and Assembly](DebuggerDisassemblerPlugin/DebuggerDisassemblerPlugin.md)
        - [Stack](DebuggerStackPlugin/DebuggerStackPlugin.md)
        - Breakpoints
            - [In the Listings](DebuggerBreakpointMarkerPlugin/DebuggerBreakpointMarkerPlugin.md)
        - [Memory Regions](DebuggerRegionsPlugin/DebuggerRegionsPlugin.md)
        - [Time](DebuggerTimePlugin/DebuggerTimePlugin.md)
        - Modules and Sections
            - [Static Mappings](DebuggerStaticMappingPlugin/DebuggerStaticMappingPlugin.md)
            - [Static Synchronization](DynamicStaticSynchronizationPlugin/DynamicStaticSynchronizationPlugin.md)
        - [Watches](DebuggerWatchesPlugin/DebuggerWatchesPlugin.md)
        - [Variable Hovers](VariableValueHoverPlugin/VariableValueHoverPlugin.md)
        - [Control and Machine State](DebuggerControlPlugin/DebuggerControlPlugin.md)
        - [Memview Plot](DebuggerMemviewPlugin/DebuggerMemviewPlugin.md)
        - [Time Overview Sidebar](DebuggerTimeOverviewPlugin/DebuggerTimeOverviewPlugin.md)
        - [P-code Stepper](DebuggerPcodeStepperPlugin/DebuggerPcodeStepperPlugin.md)
        - [Comparing Times](DebuggerTraceViewDiffPlugin/DebuggerTraceViewDiffPlugin.md)
        - [Platform Selection](DebuggerPlatformPlugin/DebuggerPlatformPlugin.md)
    - Decompiler
        - Decompiler Concepts
            - [P-code](DecompilePlugin/DecompilerConcepts/#p-code)
            - [The HighFunction](DecompilePlugin/DecompilerConcepts/#the-highfunction)
            - [SLEIGH Specification Files](DecompilePlugin/DecompilerConcepts/#sleigh-specification-files)
        - Program Annotations Affecting the Decompiler
            - [Machine Instructions](DecompilePlugin/DecompilerAnnotations/#machine-instructions)
            - [Comments](DecompilePlugin/DecompilerAnnotations/#comments)
            - [Variable Annotations](DecompilePlugin/DecompilerAnnotations/#variable-annotations)
            - [Function Prototypes](DecompilePlugin/DecompilerAnnotations/#function-prototypes)
            - [Data Mutability](DecompilePlugin/DecompilerAnnotations/#data-mutability)
            - [Constant Annotations](DecompilePlugin/DecompilerAnnotations/#constant-annotations)
            - [Register Values](DecompilePlugin/DecompilerAnnotations/#register-values)
        - Decompiler Options
            - [General Options](DecompilePlugin/DecompilerOptions/#general-options)
            - [Analysis Options](DecompilePlugin/DecompilerOptions/#analysis-options)
            - [Display Options](DecompilePlugin/DecompilerOptions/#display-options)
            - [Program Options](DecompilePlugin/DecompilerOptions/#program-options)
            - [Specification Extensions](DecompilePlugin/DecompilerOptions/#specification-extensions)
        - Decompiler Window
            - [Display](DecompilePlugin/DecompilerWindow/#display)
            - [Main Window](DecompilePlugin/DecompilerWindow/#main-window)
            - [Snapshot Windows](DecompilePlugin/DecompilerWindow/#snapshot-windows)
            - [Undefined Functions](DecompilePlugin/DecompilerWindow/#undefined-functions)
            - [Tool Bar](DecompilePlugin/DecompilerWindow/#tool-bar)
            - [Mouse Actions](DecompilePlugin/DecompilerWindow/#mouse-actions)
            - [Pop-up Menu and Keyboard Actions](DecompilePlugin/DecompilerWindow/#pop-up-menu-and-keyboard-actions)
        - [Decompiler Taint](DecompilerTaint/DecompilerTaint/#decompiler-taint-operations)
    - [Eclipse Integration](EclipseIntegration/EclipseIntegration.md)
    - Entropy Overview
        - [Entropy Calculation](OverviewPlugin/Overview/#calculation-of-entropy)
        - [Data Encoding](OverviewPlugin/Overview/#data-encoding)
        - [Color Palette](OverviewPlugin/Overview/#color-palette-legend)
        - [Configuration Options](OverviewPlugin/Overview/#configuring-the-entropy-window)
    - [FileSystem Browser](FileSystemBrowserPlugin/FileSystemBrowserPlugin.md)
    - Function Bit Patterns Explorer
        - [Byte Sequences](FunctionBitPatternsExplorerPlugin/FunctionBitPatternsExplorerPlugin/#byte-sequence-tabs)
        - [Instruction Sequences](FunctionBitPatternsExplorerPlugin/FunctionBitPatternsExplorerPlugin/#instruction-sequence-tabs)
        - [Function Start Alignment](FunctionBitPatternsExplorerPlugin/FunctionBitPatternsExplorerPlugin/#function-start-alignment-tab)
        - [Context Register Information](FunctionBitPatternsExplorerPlugin/FunctionBitPatternsExplorerPlugin/#context-register-information-tab)
        - [Pattern Clipboard](FunctionBitPatternsExplorerPlugin/FunctionBitPatternsExplorerPlugin/#pattern-clipboard)
    - Function ID
        - Function ID Analyzer
            - [Analysis Options](FunctionID/FunctionID/#analysis-options)
            - [Scoring and Disambiguation](FunctionID/FunctionID/#scoring-and-disambiguation)
        - Function ID Plugin
            - [Enabling the Plug-in](FunctionID/FunctionIDPlugin/#enabling-the-plug-in)
            - [Plug-in Functionality](FunctionID/FunctionIDPlugin/#plug-in-functionality)
            - [Preparing Libraries for a Function ID Database](FunctionID/FunctionIDPlugin/#preparing-libraries-for-a-function-id-database)
            - [False Positives](FunctionID/FunctionIDPlugin/#false-positives)
        - Function ID Debug Plugin
            - [Enabling the Plug-in](FunctionID/FunctionIDDebug/#enabling-the-plug-in)
            - [Plug-in Functionality](FunctionID/FunctionIDDebug/#plug-in-functionality)
    - Graphing
        - Graph Services
            - [Default Graph Display](GraphServices/GraphDisplay.md)
            - [Exporting a Graph](GraphServices/GraphExport.md)
        - Data Graph
            - Vertices
                - [Vertex Grouping](FunctionGraphPlugin/Function_Graph/#vertex-grouping)
            - [Graph Actions](DataGraphPlugin/Data_Graph/#data-graph-actions)
            - [Popups](DataGraphPlugin/Data_Graph/#vertex-actions)
            - [Layout](DataGraphPlugin/Data_Graph/#vertex-layout)
        - [Function Call Graph](FunctionCallGraphPlugin/Function_Call_Graph.md)
        - Function Graph
            - [Primary View](FunctionGraphPlugin/Function_Graph/#primary-view)
            - [Satellite View](VisualGraph/Visual_Graph/#satellite-view)
            - [Zooming](FunctionGraphPlugin/Function_Graph/#zooming)
            - [Layouts](FunctionGraphPlugin/Function_Graph_Layouts/#function-graph-layouts)
        - [Program Graph](ProgramGraphPlugin/ProgramGraph.md)
        - [Visual Graphs](VisualGraph/Visual_Graph.md)
    - Memory Map
        - [Editing a Memory Block](MemoryMapPlugin/Memory_Map/#memory-block-edits)
        - Operations on Memory Blocks
            - [Add](MemoryMapPlugin/Memory_Map/#add)
            - [Move](MemoryMapPlugin/Memory_Map/#move)
            - [Split](MemoryMapPlugin/Memory_Map/#split)
            - [Expand Up](MemoryMapPlugin/Memory_Map/#expand-up)
            - [Expand Down](MemoryMapPlugin/Memory_Map/#expand-down)
            - [Merge](MemoryMapPlugin/Memory_Map/#merge)
            - [Delete](MemoryMapPlugin/Memory_Map/#delete)
            - [Set Image Base](MemoryMapPlugin/Memory_Map/#set-image-base)
    - Program Annotation
        - [Address Expressions](Misc/AddressExpressions.md)
        - Auto Analysis
            - [Forcing Analysis](AutoAnalysisPlugin/AutoAnalysis/#auto-analysis)
            - [Auto Analysis Options Panel](AutoAnalysisPlugin/AutoAnalysis/#auto-analysis-options-panel)
            - Implemented Auto Analyzers
                - [Entry Point Analyzer](AutoAnalysisPlugin/AutoAnalysis/#entry-point-analyzer)
                - [Subroutine Analyzer](AutoAnalysisPlugin/AutoAnalysis/#subroutine-reference-analyzer)
                - [Scalar Operand Analyzer](AutoAnalysisPlugin/AutoAnalysis/#scalar-operand-analyzer)
                - [Data Reference Analyzer](AutoAnalysisPlugin/AutoAnalysis/#data-reference-analyzer)
                - [MIPS address markup Analyzer](AutoAnalysisPlugin/AutoAnalysis/#mips-address-markup-analyzer)
                - [PowerPC Address Markup Analyzer](AutoAnalysisPlugin/AutoAnalysis/#powerpc-address-markup-analyzer)
                - [ARM Analyzer](AutoAnalysisPlugin/AutoAnalysis/#arm-analyzer)
                - [Data Archive Analyzer](AutoAnalysisPlugin/AutoAnalysis/#data-archive-analyzer)
                - [Demangler Analyzer](AutoAnalysisPlugin/AutoAnalysis/#demangler-analyzer)
                - [Decompiler Parameter ID Analyzer](AutoAnalysisPlugin/AutoAnalysis/#decompiler-parameter-id-analyzer)
                - [Windows x86 Analyzer](AutoAnalysisPlugin/AutoAnalysis/#windows-x86-pe-analyzers)
                - [Address Table Analyzer](AutoAnalysisPlugin/AutoAnalysis/#address-table-analyzer)
                - [Aggressive Instruction Finder](AutoAnalysisPlugin/AutoAnalysis/#aggressive-instruction-finder)
                - [Condense Filler Bytes](AutoAnalysisPlugin/AutoAnalysis/#condense-filler-bytes-analyzer)
        - Disassembly
            - [Dynamic Disassembly](DisassemblerPlugin/Disassembly/#disassembly)
            - [Static Disassembly](DisassemblerPlugin/Disassembly/#static-disassembly)
            - [Restricted Disassembly](DisassemblerPlugin/Disassembly/#restricted-disassembly)
            - [Processor Options](DisassemblerPlugin/Disassembly/#processor-options)
        - Data
            - [Create Data](DataPlugin/Data.md)
            - [Data Types](DataPlugin/Data/#data-types)
            - Translate Strings
                - [LibreTranslate](LibreTranslatePlugin/LibreTranslatePlugin.md)
            - [Charsets](Charsets/Charsets.md)
            - [Save Image](ResourceActionsPlugin/ResourceActions.md)
        - Labels
            - [Label Source](LabelMgrPlugin/Labels/#label-source)
            - [Default Label](LabelMgrPlugin/Labels/#default-labels)
            - [Label Properties](LabelMgrPlugin/Labels/#label-properties)
            - [Add/Edit Labels](LabelMgrPlugin/Labels/#addedit-label-dialog)
            - [Set Label](LabelMgrPlugin/Labels/#set-label-dialog)
            - Label Operations
                - [Adding a Label](LabelMgrPlugin/Labels/#adding-a-label)
                - [Renaming a Label](LabelMgrPlugin/Labels/#renaming-a-label)
                - [Removing a Label](LabelMgrPlugin/Labels/#removing-a-label)
                - [Setting the Namespace](LabelMgrPlugin/Labels/#setting-the-namespace)
                - [Setting an External Entry Point](LabelMgrPlugin/Labels/#setting-an-external-entry-point)
                - [Making a Label Primary](LabelMgrPlugin/Labels/#making-a-label-primary)
                - [Setting a Label in an Operand](LabelMgrPlugin/Labels/#selecting-a-referenced-label-for-an-operand)
                - [Show Label History](LabelMgrPlugin/Labels/#show-label-history)
        - Comments
            - [Add or Edit Comments](CommentsPlugin/Comments/#adding-or-editing-comments-set-comment)
            - [Delete Comments](CommentsPlugin/Comments/#deleting-comments)
            - [Navigating in Codebrowser through Comments](CommentsPlugin/Comments/#navigating-in-the-code-browser)
            - [Displaying Comment History](CommentsPlugin/Comments/#displaying-comment-history)
        - References
            - Forward References
                - Memory References
                    - [Offset References](ReferencesPlugin/References_from/#memory-references)
                    - [Offcut References](ReferencesPlugin/References_from/#memory-references)
                - [Stack References](ReferencesPlugin/References_from/#stack-references)
                - [Register References](ReferencesPlugin/References_from/#register-references)
                - [External References](ReferencesPlugin/References_from/#external-references)
            - [Back References](LocationReferencesPlugin/Location_References.md)
        - Equates
            - [Set Equate](EquatePlugin/Equates/#set-equate)
            - [Remove Equate](EquatePlugin/Equates/#remove-equate)
            - [Apply Enum](EquatePlugin/Equates/#apply-enum)
            - [View Equates](EquatePlugin/Equates/#view-equates)
            - Convert
                - [Signed Decimal](EquatePlugin/Equates/#signed-decimal)
                - [Unsigned Decimal](EquatePlugin/Equates/#unsigned-decimal)
                - [Octal](EquatePlugin/Equates/#unsigned-octal)
                - [Signed Hex](EquatePlugin/Equates/#signed-hex)
                - [Unsigned Hex](EquatePlugin/Equates/#unsigned-hex)
                - [Char / Char Sequence](EquatePlugin/Equates/#char-char-sequence)
                - [Binary](EquatePlugin/Equates/#unsigned-binary)
        - Functions
            - [Create Function](FunctionPlugin/Functions/#create-function)
            - [Re-create Function](FunctionPlugin/Functions/#re-create-function)
            - [Thunk Functions](FunctionPlugin/Functions/#thunk-functions)
            - [External Functions](FunctionPlugin/Functions/#external-functions)
            - [Create Multiple Functions](FunctionPlugin/Functions/#create-multiple-functions)
            - [Rename Function](FunctionPlugin/Functions/#rename-function)
            - [Delete Function](FunctionPlugin/Functions/#delete-function)
            - [Function Purge](FunctionPlugin/Functions/#function-purge)
            - [Function Repeatable Comment](FunctionPlugin/Functions/#function-repeatable-comment)
            - [Stack Depth Change](FunctionPlugin/Functions/#stack-depth-change)
            - [Function Signature and Variables](FunctionPlugin/Variables.md)
            - [Function Tags](FunctionTagPlugin/function_tag_window.md)
        - [Set Register Values](RegisterPlugin/Registers.md)
        - [Bookmarks](BookmarkPlugin/Bookmarks.md)
        - [Clear](ClearPlugin/Clear.md)
        - [View Properties](PropertyManagerPlugin/Property_Viewer.md)
        - [Set Fallthrough Address](FallThroughPlugin/Override_Fallthrough.md)
        - Navigation
            - [Go to Address Label](Navigation/Navigation.md)
            - [Go to Next/Previous](Navigation/Navigation/#navigation-history)
            - [Margin and Navigation Markers](CodeBrowserPlugin/CodeBrowser/#markers)
        - Selection
            - [Selection by Flow](FlowSelection/Selection_By_Flow.md)
            - [Selection Highlighting](SetHighlightPlugin/Highlighting.md)
        - Program Search
            - [Memory](Search/Search_Memory.md)
            - [Text](Search/Search_Program_Text.md)
            - [Search and Replace](Search/SearchAndReplace.md)
            - [Strings](Search/Search_for_Strings.md)
            - [Decompiled Text](DecompilerTextFinderPlugin/Decompiler_Text_Finder.md)
            - [Scalars](ScalarSearchPlugin/The_Scalar_Table.md)
            - [Instruction Patterns](Search/Search_Instruction_Patterns.md)
            - [Address Tables](Search/Search_for_AddressTables.md)
            - [Direct References](Search/Search_for_DirectReferences.md)
            - [Query Results Window](Search/Query_Results_Dialog.md)
        - [DWARF External Debug Files](DWARFExternalDebugFilesPlugin/DWARFExternalDebugFilesPlugin.md)
        - PDB
            - [Load PDB File](Pdb/LoadPDB.md)
            - [PDB Parser (README_PDB)](docs/README_PDB.md)
        - [SARIF](Sarif/SARIF.md)
    - Program Tree
        - [Folders and Fragments](ProgramTreePlugin/program_tree/#folders-and-fragments)
        - [Control the View](ProgramTreePlugin/program_tree/#control-the-view-in-the-code-browser)
        - [Cut/Copy/Paste and Drag and Drop](ProgramTreePlugin/program_tree/#cutcopypaste-and-drag-and-drop)
        - [Program Organizations](ProgramTreePlugin/Program_Organizations.md)
        - [Create view](ProgramTreePlugin/view_manager/#create-a-default-program-tree)
        - [Delete view](ProgramTreePlugin/view_manager/#delete-a-program-tree)
        - [Rename view](ProgramTreePlugin/view_manager/#rename-a-program-tree)
        - [Close view](ProgramTreePlugin/view_manager/#close-a-program-tree)
    - [Program Validator](ValidateProgram/ValidateProgram.md)
    - [Relocation Table](RelocationTablePlugin/relocation_table.md)
    - Scripting
        - [Listing Scripts by Category](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#script-category-tree)
        - [Information in Script Table](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#script-table)
        - [Finding Scripts with Search Filter](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#filter)
        - Script Manager Actions
            - [Run a Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#run-script)
            - [Edit a Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#edit-script)
            - [Edit a Script with Eclipse](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#edit-script-with-eclipse)
            - [Save a Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#save)
            - [Save a Script As...](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#save-as)
            - [Undo the Actions of a Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#undo-undo)
            - [Redo the Actions of a Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#redo-redo)
            - [Select Font in Script Editor](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#select-font)
            - [Assign Key Bindings for Scripts](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#assign-key-binding)
            - [Delete a Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#delete-script)
            - [Rename a Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#rename-script)
            - [Create a New Script](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#create-new-script)
            - [Refresh Script List](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#refresh-script-list)
            - [Add and Remove Script Directories](GhidraScriptMgrPlugin/GhidraScriptMgrPlugin/#script-directories-bundle-manager)
        - [Console](ConsolePlugin/console.md)
        - [Ghidra Script Development](GhidraScriptMgrPlugin/ScriptDevelopment.md)
        - [Ghidra Bundles](BundleManager/BundleManager.md)
        - [PyGhidra Interpreter](PyGhidra/interpreter.md)
    - Symbol Table
        - [Symbol References](SymbolTablePlugin/symbol_references.md)
        - [Symbol Tree](SymbolTreePlugin/SymbolTree.md)
        - [Temporary Symbol Table](SymbolTablePlugin/symbol_table_transient.md)
    - Version Tracking
        - Workflow
            - [Preconditions](VersionTrackingPlugin/VT_Preconditions.md)
            - [Example Workflow](VersionTrackingPlugin/VT_Workflow/#example-work-flow-match-all-possible-functions-between-two-binaries)
            - [Workflow FAQ](VersionTrackingPlugin/VT_Workflow/#workflow-faq)
            - [Common Problems](VersionTrackingPlugin/VT_Workflow/#common-problems)
        - Program Correlators
            - [BSim Program Correlator](BSimCorrelator/BSim_Correlator.md)
            - [Data Match Correlator](VersionTrackingPlugin/VT_Correlators/#data-match-correlators)
            - [Function Match Correlator](VersionTrackingPlugin/VT_Correlators/#function-match-correlators)
            - [Implied Match Correlator](VersionTrackingPlugin/VT_Correlators/#implied-correlator)
            - [Legacy Import Match Correlator](VersionTrackingPlugin/VT_Correlators/#legacy-import-correlator)
            - [Manual Match Correlator](VersionTrackingPlugin/VT_Correlators/#manual-match-correlator)
            - [Symbol Name Match Correlator](VersionTrackingPlugin/VT_Correlators/#symbol-name-match-correlators)
        - [Session Wizard](VersionTrackingPlugin/VT_Wizard.md)
        - Tool
            - [Matches Table](VersionTrackingPlugin/providers/VT_Matches_Table.md)
            - [Markup Items Table](VersionTrackingPlugin/providers/VT_Markup_Table.md)
            - [Functions Table](VersionTrackingPlugin/providers/VT_Functions_Table.md)
            - [Related Matches Table](VersionTrackingPlugin/providers/VT_Related_Associations_Table.md)
            - [Implied Matches Table](VersionTrackingPlugin/providers/VT_Implied_Matches_Table.md)
    - [Wildcard Assembler](WildcardAssemblerModule/Wildcard_Assembler.md)
- Support
    - [GhidraGo](GhidraGo/GhidraGo.md)
    - [Headless Analyzer](HeadlessAnalyzer/HeadlessAnalyzer.md)
- [Keyboard Navigation](KeyboardNavigation/KeyboardNavigation.md)
- [Undo/Redo](Tool/Undo_Redo.md)
- [Glossary](Glossary/glossary.md)
- [Tips of the Day](Misc/Tips/#ghidra-tips-of-the-day)
- Theming
    - [User's Guide](Theming/ThemingUserDocs.md)
    - [Developer's Guide](Theming/ThemingDeveloperDocs.md)
    - [Architecture](Theming/ThemingInternals.md)
- Appendix
    - [Block Models](BlockModel/Block_Model.md)
    - [Languages](LanguageProviderPlugin/Languages.md)
