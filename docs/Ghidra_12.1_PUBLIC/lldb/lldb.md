[Home](../index.md) > [lldb](index.md) > Via SSH (to Windows cmd shell)
...
15 or later.
- **`lldb` cmd args**: Arguments passed directly to LLDB (versus the target).
...
This works the same as the [GDB via SSH (to Windows cmd shell)](../gdb/gdb.md#via-ssh-to-windows-cmd-shell)
launcher, but runs `lldb` on a remote Windows system via `ssh`.
...
necessary for connecting to the Android debugger, but uses **Device** in place of **Host**
(typically retrieved using 'adb devices').
...
This has the same options as the [Andoid](#android) launcher, but uses the
target's **Process Id** instead of the executable **Image**.  **Additional commands** has been added
to allow commands like 'process handle --pass true --stop false --notify false SIGSEGV'
to be saved and executed for each run.
