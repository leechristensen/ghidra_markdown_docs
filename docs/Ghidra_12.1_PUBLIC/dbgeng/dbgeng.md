...
"`ssh-dbgeng`" is the remote equivalent to "`dbgeng`";
"`ssh-dbgeng-attach`" is the remote equivalent to "`dbgeng-attach`".
...
listen for dbgeng's Trace RMI connection and forward it back to Ghidra.
...
[← Previous: TTD (Time-Travel Debugging)](dbgeng.md) | [Next: x64dbg integration via x64dbg_automate →](../x64dbg/x64dbg.md)
